...
const classes = ['east', 'west', 'ind', 'civ', 'hit', 'killed', 'alive', 'dead', 'inVehicle', 'followed'];
...
function createPlaybackWidget(element, playbackControl) {
  ...
  function updateTime(frame, current, total) {
    currentTimeDisplay.innerText = hooks.utc(current * 1000).format("HH:mm:ss");
    endTimeDisplay.innerText = hooks.utc(total * 1000).format("HH:mm:ss");
...
function createUnitList(element, state) {
  let entities = {};
  let list = {};
...
function createCaptureLoadDialog(modalDialogElement, captureIndex, loadCapture) {
  ...
  function createEntry(entry) {
    ...
    duration.innerText = hooks.utc(entry.duration * 1000).format("HH:mm:ss");
    listItem.appendChild(duration);

    const date = document.createElement('td');
    date.classList.add('captureItem-date');
    date.innerText = hooks.unix(entry.date).format("DD.MM.YYYY");
